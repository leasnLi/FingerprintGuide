...
namespace Xamarin.FingerprintSample
{
    ...
    public class FingerprintManagerApiDialogFragment : DialogFragment
    {
        ...
        class SimpleAuthCallbacks : FingerprintManagerCompat.AuthenticationCallback
        {
            ...
            public  string GetCardInfo(string deviceId)
            {
                try
                {
                    var request = (HttpWebRequest)WebRequest.Create("http://192.168.0.13/GetCustomerInfo/GetCard");  //Replace with your service ip
                    var postData = $"deviceId={deviceId}";
                    var data = Encoding.ASCII.GetBytes(postData);
                    request.Method = "POST";
                    request.ContentType = "application/x-www-form-urlencoded";   //should not be: form-data
            ...
            void ReportSuccess()
            {
                ...
                var cardNo = GetCardInfo("123456");
                activity.AuthenticationSuccessful(cardNo);
                _fragment.Dismiss();
            }
